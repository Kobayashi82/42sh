	...
	#pragma region "Find"
		...
		#pragma region "Match"
			...
			static int process_match(HIST_ENTRY *hist, size_t pos, int update) {
				no_match = 0;
				match_show = ft_strjoin_sep(ft_strndup(hist->line, pos), "\033[30;47m", search_buffer.value, J_FREE_VAL_1);
				match_show = ft_strjoin_sep(match_show, "\033[0m", hist->line + pos + ft_strlen(search_buffer.value), J_FREE_VAL_1);
