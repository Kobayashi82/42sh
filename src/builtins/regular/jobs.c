...
#pragma region "Help / Version"
	...
	#pragma region "Help"
		...
		int bt_jobs_help(int format, int no_print) {
			char *name = "jobs";
			char *syntax = "jobs [-lnprs] [job_spec ...] or jobs -x command [args]";
			char *description = "Display status of jobs.";
			char *msg =
				"    Lists the active jobs.\n"
				"    JOB_SPEC restricts output to that job.\n"
...
#pragma region "Jobs"
	...
	int bt_jobs(int argc, char **argv) {
		...
		t_parse_result *result = parse_options(argc, argv, "lnprsx", NULL, long_opts, "jobs [-lnprs] [job_spec ...] or jobs -x command [args]", 0);
		if (!result)		return (1);
