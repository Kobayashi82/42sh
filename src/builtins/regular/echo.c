...
/*   Updated: 2026/01/06 22:26:30 by vzurera-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
#pragma region "Echo"
	...
	int bt_echo(int argc, char **argv) {
		...
		t_parse_result *result = parse_options(argc, argv, "neE", NULL, long_opts, "echo [-neE] [arg ...]", 2);
		if (!result)		return (1);
		...
		for (int i = 0; i < result->argc; ++i) {
			...
			print(STDOUT_FILENO, result->argv[i],               JOIN);
			if (i + 1 < result->argc) print(STDOUT_FILENO, " ", JOIN);
		}

		if (has_option(result, 'n'))	print(STDOUT_FILENO, NULL, PRINT);
