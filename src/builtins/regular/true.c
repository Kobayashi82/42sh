...
/*   Updated: 2026/01/24 16:52:44 by vzurera-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
#pragma region "Help / Version"
	...
	#pragma region "Help"
		...
		int bt_true_help(int format, int no_print) {
			char *name = "true";
			char *syntax = "true";
			char *description = "Return a successful result.";
			char *msg =
				"    Exit Status:\n"
				"      Always succeeds.\n";
...
#pragma region "True"
	...
	int bt_true(int argc, char **argv) {
		...
		t_parse_result *result = parse_options(argc, argv, NULL, NULL, long_opts, "true", IGNORE_OFF);
		if (!result) return (free_options(result), (shell.error == E_OPT_MAX || shell.error == E_OPT_INVALID) ? 2 : 1);

		if (find_long_option(result, "help"))		return (free_options(result), bt_true_help(HELP_NORMAL, 0));
