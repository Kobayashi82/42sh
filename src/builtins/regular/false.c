...
/*   Updated: 2026/01/24 16:52:31 by vzurera-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
#pragma region "Help / Version"
	...
	#pragma region "Help"
		...
		int bt_false_help(int format, int no_print) {
			char *name = "false";
			char *syntax = "false";
			char *description = "Return an unsuccessful result.";
			char *msg =
				"    Exit Status:\n"
				"      Always fails.\n";
...
#pragma region "False"
	...
	int bt_false(int argc, char **argv) {
		...
		t_parse_result *result = parse_options(argc, argv, NULL, NULL, long_opts, "false", IGNORE_OFF);
		if (!result) return (free_options(result), (shell.error == E_OPT_MAX || shell.error == E_OPT_INVALID) ? 2 : 1);

		if (find_long_option(result, "help"))		return (free_options(result), bt_false_help(HELP_NORMAL, 0));
