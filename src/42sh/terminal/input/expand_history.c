...
/*   Updated: 2025/02/25 21:34:10 by vzurera-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
#pragma endregion
	...
	#pragma region "Value"
		...
		static char *expand_value(char *value, int start, size_t *end, size_t length, bool at_start) {
			...
			if (at_start) {
				while (!ft_isspace(value[*end]) && *end < length) (*end)++;
				sub_value = ft_substr(value, start + 1, *end - (start + 1));
			} else {
				(*end)++;
				while (value[*end] != '?' && !ft_isspace(value[*end]) && *end < length) (*end)++;
	...
	#pragma region "Expand"
		...
		void expand_history(char **input) {
			if (!options.history || input || !*input) return;
			...
			while (i < length) {
				...
				if (value[i] == '!' && i + 1 < length && !ft_isspace(value[i + 1]) && !in_quotes && !in_dquotes) {
					int start = i;
					size_t end = i + 1;
					...
					if (value[end] == '!') {																						// !!
						...
						if (!replacement) return;
					} else if (value[end] == '?' && value[end + 1] != '?' && end + 1 < length && !ft_isspace(value[end + 1])) {		// !?[str], !?[str]?
