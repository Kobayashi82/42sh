...
/*   Updated: 2025/11/18 11:10:34 by vzurera-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	void			*ptr, **hashtable, **mem, **new_mem;
	unsigned int	i;

	if (!(ptr = __real_malloc(size))) return (NULL);
	hashtable = (void **)hash_index(NULL);
	if (!hashtable) return (NULL);
	if ((mem = mem_find(ptr))) return (__real_free(*mem), *mem = ptr, ptr);
	i = (uintptr_t)hash_index(ptr) % MEM_HASH_SIZE;
	if (!(new_mem = __real_malloc(sizeof(void *) * 2))) return (__real_free(ptr), NULL);
	new_mem[0] = ptr;
