		...
		int expand_alias(char **input, t_context *context) {
			...
			while ((*input)[i]) {
				...
				if (context->in_escape) {
					i += 1; context->in_escape = 0; continue;
				} else if ((*input)[i] == '\\' && (!context->stack || context->stack->type != CTX_QUOTE)) {
					strcpy(last_token, "\\");
					i += 1; context->in_escape = 1; continue;
				}
