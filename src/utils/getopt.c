...
#pragma region "Internal"
	...
	#pragma region "Parse Short"
		...
		static int parse_short_options(t_parse_result *result, char **argv, int *idx, int argc, const char *short_opts, int is_plus) {
	...
	#pragma region "Parse Options"
		...
		t_parse_result *parse_options(int argc, char **argv, const char *short_opts, const char *short_opts_plus, const t_long_option *long_opts, const char *usage) {
			...
			for (int i = 1; i < argc; i++) {
				...
				if (arg[0] == '-' && arg[1]) {
					if (max_options(result, opt_count++)) break;									// Error
					if (parse_short_options(result, argv, &i, argc, short_opts, 0) < 0) break;		// Error
					continue;
				}
				...
				if (arg[0] == '+' && arg[1]) {
					if (max_options(result, opt_count++)) break;									// Error
					if (parse_short_options(result, argv, &i, argc, short_opts_plus, 1) < 0) break;	// Error
					continue;
				}
